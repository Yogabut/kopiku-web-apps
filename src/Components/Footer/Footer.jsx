...
function Footer() {
  return (
    <footer className="absolute w-full bg-[#2F5249]" id="footer">
      <div className="mx-auto w-full max-w-7xl px-8 pt-10">
        <div className="grid grid-cols-1 justify-between gap-4 md:grid-cols-2">
          <Typography variant="h2" className="mb-6 text-gray-300">
            Kopiku Cafe
          </Typography>
          <div className="grid grid-cols-3 justify-between gap-4">
            {LINKS.map(({ title, items }) => (
              <ul key={title}>
                <Typography
                  variant="small"
                  className="mb-3 font-medium text-gray-300 opacity-90"
                >
                  {title}
                </Typography>
                {items.map((link) => (
                  <li key={link}>
                    <Typography
                      as="a"
                      href="#"
                      className="py-1.5 font-normal text-gray-300 transition-colors hover:text-white"
                    >
                      {link}
                    </Typography>
                  </li>
        ...
        <div className="mt-12 flex w-full flex-col items-center justify-center border-t border-gray-500 py-4 md:flex-row md:justify-between">
          ...
          >
            &copy; {currentYear} <a href="https://material-tailwind.com/">Kopiku Cafe</a>. All
            Rights Reserved.
          </Typography>
